...
def vary_data(dim, unfix1, unfix2, file_loc=r"16Mdata.xls"):
    # X data generation
    excel = xlrd.open_workbook(file_loc)
...
def compute_accuracy(model, likelihood, test_x, test_y, train_x, train_y):
    ...
    return accuracy, accuracy2, acc0, acc1
...
for d in range(24,23,-1):
    ...
    cluster_list, acc_list, acc_list2, state_dict_list, mll_list, acc0_list, acc1_list = [], [], [], [], [], [], []
    ...
    for i in range(5):
        ...
        with gpytorch.settings.max_cholesky_size(2000):
            model, mll1 = make_and_fit_classifier(train_x, train_y, inbuffer=inbuffer,
                                                  lr=0.1)
            acc, acc2, acc0, acc1 = compute_accuracy(model, model.likelihood, test_x, test_y,
                                         train_x, train_y)
        ...
        unfix1 = 3
        unfix2 = 8
        fixed = vary_data(dim=24, unfix1=unfix1, unfix2=unfix2)
        d1 = []
        d2 = []
        for j in range(21):
            d1.append([])
        ...
        for j1 in range(3):
            fixed[12]=0.25+0.25*float(j1)
            for j2 in range(3):
                fixed[17]=0.25+0.25*float(j2)
                for j3 in range(3):
                    fixed[16]=0.25+0.25*float(j3)
                    for j4 in range(3):
                        fixed[5]=0.25+0.25*float(j4)
                        for j5 in range(3):
                            fixed[20]=0.25+0.25*float(j5)
                            for j in range(21):
                                fixed[unfix1]=float(j)/20.
                                for k in range(21):
                                    fixed[unfix2]=float(k)/20.
    ...
    post0 = posterior0.data.div(1215.)
    post1 = posterior1.data.div(1215.)
    prob0 = prob0.data.div(1215.)
    prob1 = prob1.data.div(1215.)
