...
def generate_data(dim, file_loc=r"C:\Users\ASUS\Desktop\16Mdata.xls"):
    # X data generation
    ...
    # add
    excel1 = xlrd.open_workbook(r"C:\Users\ASUS\Desktop\atomic.xls")
    ...
    excel2 = xlrd.open_workbook(r"C:\Users\ASUS\Desktop\xenonpy.xls")
...
def vary_data(dim, unfix1, unfix2, file_loc=r"C:\Users\ASUS\Desktop\16Mdata.xls"):
    # X data generation
    excel = xlrd.open_workbook(file_loc)
    ...
    for i in range(len(matname)):
        ...
        for k in range(len(ele1)):
            if sqatom[i] == ele1[k]:
                ...
                rcsq = rcov1[k]
        datapoint =[rmax, enmin, eamin, eamax, rmin, dsq[i], dv[i], easq, cdata[i], ipsq]
        #[rmax, enmin, eamin, emax, rmin, dsq[i], dv[i], easq, cdata[i], ipsq]
        for j in range(len(databuffer)):
            datapoint.append(max(databuffer[j]))
            datapoint.append(min(databuffer[j]))
            datapoint.append(sqbuffer[j][0])
    ...
    return torch.tensor(Xtot, dtype=torch.float32)
...
class TestModel(ExactGP):
    def __init__(self, train_x, train_y, likelihood, num_classes, initialization, rank=4, interval1=10.):
        super(DirichletGPModel, self).__init__(train_x, train_y, likelihood)
        self.mean_module = ConstantMean(batch_shape=torch.Size((num_classes,)))
        self.covar_module_projection = RBFKernel(
            ard_num_dims=rank,
...
basis=torch.randn(25, 3, dtype=torch.float32)/3.
for d in range(25,24,-1):
    ...
    for i in range(d):
        inbuffer.append([])
        for j in range(len(basis[i+25-d])):
            inbuffer[i].append(basis[i+25-d][j])
    ...
    for i in range(1):
        ...
        with torch.no_grad():
            """for j in range(25):
                if (j != unfix1) and (j != unfix2):
                    for k in range(3):
                        model.projection[j][k] = 0.
                else:
                    model.projection[5]=torch.tensor([-1.3284, -2.2238, -0.5832],dtype=torch.float64)
                    model.projection[6] = torch.tensor([2.7761,4.1011, 2.7382], dtype=torch.float64)

            for i in range(25):
                if (i!=unfix1) and (i!=unfix2):
                    model.covar_module_ard.lengthscale[0][i]=100.
                else:
                    model.covar_module_ard.lengthscale[0][5]=0.1824
                    model.covar_module_ard.lengthscale[0][6]=1.5038
            model.covar_module_projection.lengthscale[0]=torch.tensor([0.3333, 0.1304, 0.3719], dtype=torch.float64)
            print(model.covar_module_ard.lengthscale)
            print(model.projection)"""
            model.eval()

            reducedata=vary_data(dim=2,unfix1=unfix1,unfix2=unfix2)
            reducedata=reducedata.to(device, dtype)
            likelihood1 = DirichletClassificationLikelihood(true_y[trainset], learn_additional_noise=True)
            model1 = DirichletGPModel(
                reducedata[trainset], likelihood1.transformed_targets, likelihood1, num_classes=likelihood1.num_classes,
            ...
            """for k in range(21):
                d1.append(float(k) / 20.)
                d2.append(float(k) / 20.)
            for j in range(21):
                # d1.append([])
                # d2.append([])
                value1 = float(j) / 20.
                ...
                for k in range(21):
                    ...
                    prob0[k][j] = prob0[k][j] + probabilities[0]/(probabilities[0]+probabilities[1])
                    prob1[k][j] = prob1[k][j] + probabilities[1]/(probabilities[0]+probabilities[1])
            ...
            post0 = posterior0.data.div(1.)
            post1 = posterior1.data.div(1.)
            prob0 = prob0.data.div(1.)
            prob1 = prob1.data.div(1.)
            levels = np.linspace(0., 1.05, 50)
            print('prob0', prob0)
            print('prob1', prob1)
            rmatrix0, smatrix0, cvector10 = cluster_lengthscales(model)
            plt.figure()
            plt.imshow(rmatrix0.cpu())
            plt.colorbar()
            plt.figure()
            im = plt.contourf(d1, d2, prob0,levels=levels)
            plt.colorbar(im)
            plt.savefig('Prob0.png')
            plt.show()
            plt.figure()
            ...
            plt.colorbar(im)
            plt.show()
            plt.savefig('Posterior0.png')
            plt.figure()
            im = plt.contourf(d1, d2, post1,levels=levels)
            plt.colorbar(im)
            plt.show()
            plt.savefig('Posterior1.png')
            ...
            ax = plt.subplot()
            f = plt.imshow(summatrix1.data.div(1.), cmap=mpl.cm.bwr, vmin=-1., vmax=1.)
